...
def load_json(path: Path) -> dict[str, object]:
    if not path.is_file():
        raise VerificationError(f"missing file: {path.relative_to(REPO_ROOT)}")
...
def verify_proto_contract(manifest: dict[str, object]) -> None:
    ...
    assert_equal(
        {name: envelope_values.get(name) for name in manifest["envelopePayloads"]},
    ...
    for capability, details in manifest["capabilities"].items():
        ...
        assert_equal(capability_values.get(capability), expected_value, capability)
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(f"shared protocol model verified: {args.manifest.relative_to(REPO_ROOT)}")
    return 0
