...
This command is intentionally narrow. coturn's TURN REST username is not the
Authority allocation ID, so every production use must provide a trusted registry
that binds the Authority allocation to the coturn username and, when needed, the
native coturn session id. Missing or ambiguous bindings fail closed.
"""
...
def _read_active_sessions(settings: CLISettings, runner: Callable[[CLISettings, str], str]) -> tuple[CoturnSession, ...]:
...
def add_cli_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--registry", type=Path, help=f"strict allocation registry JSON; defaults to {DEFAULT_REGISTRY_ENV}")
    ...
    parser.add_argument("--cli-password-env", default=DEFAULT_CLI_PASSWORD_ENV, help="environment variable containing the coturn CLI password")
    parser.add_argument("--cli-password-file", type=Path, help=f"file containing the coturn CLI password; defaults to {DEFAULT_CLI_PASSWORD_FILE_ENV}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    export.add_argument("--state-file", type=Path, help=f"persistent monotonic sequence state; defaults to {DEFAULT_SEQUENCE_STATE_ENV}")
