...
RELAY_CREDENTIAL_FIELDS = frozenset(
    {
        "new_credential_rejected",
        "new_credential_status",
        "stale_credential_reuse_rejected",
        "stale_credential_status",
...
def verify_report(report: dict[str, Any]) -> VerificationResult:
    ...
    relay_credential = _object(
        report.get("relay_credential"), "relay_credential", RELAY_CREDENTIAL_FIELDS
    )
    ...
    ttl = _optional_positive_int(
        relay_credential, "credential_ttl_seconds", "relay_credential.credential_ttl_seconds"
    )
