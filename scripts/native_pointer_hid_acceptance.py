...
def read_device_identity(serial: str) -> DeviceIdentity:
    devices = run_command(["adb", "-s", serial, "devices", "-l"]).stdout.splitlines()
...
def create_lock_blocked_result(
    *,
    created_at: str,
    requested_serial: str,
...
def write_result(path: Path, result: AcceptanceResult, dumpsys_input: str) -> None:
    ...
    if result.existing_locks:
        ...
        for lock in result.existing_locks:
            summary.append(f"- `{lock.path}`: {lock.detail}")
        summary.append("")
    summary.extend([
        "## Artifacts",
        "",
    ...
    if result.status == "blocked_device_coordination_lock":
        summary.append(
            "A `blocked_device_coordination_lock` result proves only that the run did not start because a shared device lock existed; it does not include device capability evidence and cannot close the native pointer HID gate."
        )
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="When a shared Android device coordination lock exists, write blocked evidence without running ADB.",
    )
    ...
    try:
        ...
        if existing_locks and not args.allow_existing_device_lock:
            if args.write_blocked_on_lock:
                result = create_lock_blocked_result(
                    created_at=created_at,
                ...
                return BLOCKED_EXIT
            raise AcceptanceError(
                "Android device coordination lock exists; no ADB command was run: "
