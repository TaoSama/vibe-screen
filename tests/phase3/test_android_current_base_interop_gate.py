...
class AndroidCurrentBaseInteropGateTests(unittest.TestCase):
    ...
    def test_cli_writes_blocked_output_before_expected_commit_is_known(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            repo = Path(directory) / "not-a-repo"
            repo.mkdir()
            evidence = Path(directory) / "evidence.json"
            output = Path(directory) / "gate.json"
            evidence.write_text(json.dumps(combined_report()), encoding="utf-8")
            ...
            written = json.loads(output.read_text(encoding="utf-8"))
            self.assertEqual(written["result"], "blocked")
            self.assertNotIn("accepted_commit", written)
            self.assertIn("cannot determine repository source state", written["reasons"][0])
