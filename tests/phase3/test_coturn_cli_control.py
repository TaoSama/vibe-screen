...
class CoturnCLIControlTests(unittest.TestCase):
    ...
    def test_parse_coturn_ps_rejects_session_id_outside_safe_identifier(self) -> None:
        with self.assertRaisesRegex(control.CoturnControlError, "malformed coturn CLI session header"):
            control.parse_coturn_ps(
                "1) id=0130:unsafe, user <1700000000:device-1>:\n"
                "  usage: rp=3, rb=140, sp=2, sb=96\n"
            )
    ...
    def test_export_command_requires_explicit_source_id(self) -> None:
        registry = self.write_registry(self.registry_payload())
        password = self.root / "cli-password.txt"
        password.write_text("p" * 32, encoding="utf-8")
        with mock.patch.dict(os.environ, {"VIBE_COTURN_CLI_PASSWORD_FILE": str(password)}, clear=True):
            result = subprocess.run(
                [
                    sys.executable,
                    str(ROOT / "scripts/phase3/coturn_cli_control.py"),
        ...
        self.assertEqual(result.returncode, 2)
        self.assertIn("--source-id or VIBE_COTURN_SOURCE_ID is required", result.stderr)

    def test_export_command_uses_same_password_charset_as_coturn_startup(self) -> None:
        with self.assertRaisesRegex(control.CoturnControlError, "coturn CLI password must contain"):
            control._load_password("VIBE_COTURN_CLI_PASSWORD", self.write_text("cli-password", "p" * 31))
        with self.assertRaisesRegex(control.CoturnControlError, "coturn CLI password must contain"):
            control._load_password("VIBE_COTURN_CLI_PASSWORD", self.write_text("cli-password", "p" * 32 + "!"))
