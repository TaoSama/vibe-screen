...
const (
	...
	requiredSchemaChecksum                       = "4a45b8c1510fb6823bab7c87ecb3b948b39d53a5b24be8a8f9eb58d4b3a7181d"
...
func (s *PostgresStore) IssueSessionProfile(ctx context.Context, request SessionProfileRequest, now time.Time) (SessionProfileResponse, error) {
	...
	err = s.transaction(ctx, func(tx pgx.Tx) error {
		...
		if err := requireRegisteredDevices(ctx, tx, request.AccountID, request.HostIdentity.DeviceID, request.ClientIdentity.DeviceID); err != nil {
			return err
		}
...
func (s *PostgresStore) createSignalingTx(ctx context.Context, tx pgx.Tx, request SignalingRequest, now time.Time) (SignalingAdmission, error) {
	if err := requireRegisteredDevices(ctx, tx, request.AccountID, request.HostDeviceID, request.ClientDeviceID); err != nil {
	...
	floorDevices := uniqueStrings([]string{request.HostDeviceID, request.ClientDeviceID})
...
func requireRegisteredDevices(ctx context.Context, tx pgx.Tx, accountID, hostDeviceID, clientDeviceID string) error {
	var suspendedAt *time.Time
	if err := tx.QueryRow(ctx, `SELECT suspended_at FROM authority_accounts WHERE account_id=$1 FOR UPDATE`, accountID).Scan(&suspendedAt); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return ErrNotFound
	...
	devices := []string{hostDeviceID, clientDeviceID}
	...
	seen := 0
	for rows.Next() {
		var deviceID, registeredAccountID string
		var revokedAt *time.Time
		if err := rows.Scan(&deviceID, &registeredAccountID, &revokedAt); err != nil {
			return err
		}
		if registeredAccountID != accountID || revokedAt != nil {
			return ErrRevoked
		}
		seen++
	}
