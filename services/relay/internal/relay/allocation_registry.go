...
func upsertAllocationRegistryEntry(path string, entry allocationRegistryEntry, sourceID string) error {
	...
	if !validIdentifier(entry.AllocationID) || !validIdentifier(entry.DeviceID) || !validIdentifier(entry.SessionID) || !validIdentifier(sourceID) || !validTurnRESTUsername(entry.Username, entry.DeviceID) {
	...
	for index := range registry.Allocations {
		...
		if current.Username == entry.Username {
			return errors.New("allocation registry contains ambiguous TURN REST username")
		}
	}
	if !found {
...
func readAllocationRegistry(path string, sourceID string) (allocationRegistry, error) {
	...
	seenUsernames := make(map[string]struct{}, len(registry.Allocations))
	for index, entry := range registry.Allocations {
		if !validIdentifier(entry.AllocationID) || !validIdentifier(entry.DeviceID) || !validIdentifier(entry.SessionID) || !validTurnRESTUsername(entry.Username, entry.DeviceID) {
		...
		if _, exists := seenUsernames[entry.Username]; exists {
			return allocationRegistry{}, fmt.Errorf("allocation registry entry %d repeats TURN REST username", index)
		}
		seenUsernames[entry.Username] = struct{}{}
	}
	return registry, nil
}
