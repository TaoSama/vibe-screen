...
| Shared USB/LAN PCM product-flow fixture | PASS | `contracts/fixtures/audio/v1/usb-lan-pcm-s16le-product-flow.json` is consumed by Android `ProtocolPcmAudioStreamTest`, `ProtocolPcmAudioPlaybackTest`, StreamClient loopback tests, Swift `AudioUsbLanPcmFixtureTests`, and the release `--audio-capture-self-test`. It pins channel `3`, PCM S16LE config/result bytes, Host packet bytes, Android write order, and cleanup expectations without claiming real playback evidence. |
