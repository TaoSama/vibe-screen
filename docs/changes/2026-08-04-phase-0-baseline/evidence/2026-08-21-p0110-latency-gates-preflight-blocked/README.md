...
This record covers the external latency gate readiness state for the rebased PR
head. It is blocked evidence only and does not close any performance gate.

Device identity is retained in `device-info.json`: nubia P0110 / pacific /
Android 16 / SDK 36 / device id `<device-id>`, build fingerprint
`<build-fingerprint>`.
