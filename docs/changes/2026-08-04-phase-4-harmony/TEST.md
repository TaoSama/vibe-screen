...
The top-level Harmony device manifest also requires the `huks_backed_secure_pairing`
gate to reference a nested `harmony-secure-pairing.json` with a matching status.
This keeps generic device logs from closing the security gate without the
specific HUKS and service-admission proof. The blocked record for this work is
under
