...
| Internet glass-to-glass latency gate | `tools/vibescreen_evidence/latency.py`, `tools/schemas/latency-evidence.schema.json`, [latency runbook](../../runbook/latency-measurement.md), [blocked record](evidence/2026-08-21-internet-latency-gate-blocked/README.md) | The `internet-glass-to-glass-sub150` profile targets 80–150 ms on healthy direct public-Internet paths and requires a single external-camera timebase, a deployed public STUN/TURN service, and an independently operated remote peer. It is separate from the trusted-LAN `lan-glass-to-glass-sub80` profile and must not accept local coturn loopback, synthetic-peer, or same-private-network runs. No real external-camera Internet latency package exists in the repository, so the gate remains blocked |
