...
- The current `/v1/usage` byte/session ledger is non-authoritative telemetry;
  ...
  coturn collector is deployed and validated. `scripts/phase3/coturn_reconcile.py`
  defines the structured snapshot submission, bounded retry loop, and disconnect
  contract. `scripts/phase3/coturn_cli_control.py` provides the minimal coturn
  CLI exporter/disconnect implementation by joining coturn `ps` output to a
  relay-written allocation registry and issuing `cs <session-id>` for mapped
  revoked/unauthorized/conflicting allocations. This is not a durable multi-node
  scheduler/WAL or provider billing reconciliation.
...
| Relay control/data plane | `services/relay/`, `deploy/phase3/`, `scripts/phase3/coturn_reconcile.py`, `scripts/phase3/coturn_cli_control.py` | Short-term credential control plane and digest-pinned coturn/relay Compose data plane exist. REST usernames map all sessions/expiries for one device to one coturn allocation-quota principal, and production peer ACLs deny private, CGNAT, link-local, ULA and other internal ranges. Relay now has explicit local/production-authority modes: production credential issuance requires `allocation_id`, calls Authority `/v1/relay/admissions`, writes an allocation registry before returning a TURN credential, and fails closed on dependency, revoked session/device, quota, conflicting allocation identity, or registry write failure. Authority closes relay-allocation ledger entries when an account is suspended, a device is revoked, or a signaling admission is invalidated, and later usage/reconciliation updates for revoked, suspended, expired, or closed allocations fail closed without advancing counters. Reconciliation classifies source-observed allocations whose Authority ledger is already closed as `revoked_allocation_ids`, and the production Compose profile runs a bounded worker that joins coturn CLI `ps` output through the registry and invokes `cs <session-id>` for unauthorized/conflicting/revoked mapped allocations. The 2026-08-20 local readiness snapshot passed coturn REST allocation/quota and production peer-ACL scripts at commit `18a6ea70d0fbf6bc187f5a7242424ad3e88cf5ee`. The new coturn CLI path is locally unit/static tested only. Public deployment, authoritative byte usage, durable multi-node scheduling/WAL, provider billing reconciliation, and production end-to-end enforcement remain gates |
...
- Relay session requests remain authenticated by a trusted control-plane bearer,
  ...
  The repository now includes a minimal coturn CLI exporter/disconnect command
  and production Compose worker backed by relay's allocation registry. Admin
  revocation blocks new credentials and later usage events; mapped active TURN
  allocations can be terminated through coturn CLI, but public deployment and
  end-to-end revocation evidence remain open.
