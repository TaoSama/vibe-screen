...
- Coturn applies `user-quota` after stripping the REST expiry, so the current
  principal is `device-id:session-id:allocation-id`. The coturn limit bounds ICE
  allocations for one Vibe Screen allocation; per-device allocation limits stay
  in Authority and the relay admission gate. Allocation lifetime, peer
  permissions, bandwidth, and concurrent allocations are bounded in the data
  plane.
...
| Relay control/data plane | `services/relay/`, `deploy/phase3/`, `scripts/phase3/coturn_exporter.py`, `scripts/phase3/coturn_reconcile.py`, `scripts/phase3/coturn_disconnect_executor.py` | Short-term credential control plane and digest-pinned coturn/relay Compose data plane exist. REST usernames encode `expiry/device/session/allocation`, allowing the coturn exporter to map active allocations back to Authority ledger fields. Production peer ACLs deny private, CGNAT, link-local, ULA and other internal ranges. Relay now has explicit local/production-authority modes: production credential issuance requires `allocation_id`, calls Authority `/v1/relay/admissions`, and fails closed on dependency, revoked session/device, quota, or conflicting allocation identity before returning a TURN credential. Authority closes relay-allocation ledger entries when an account is suspended, a device is revoked, a relay-initiated device revoke is accepted, or a signaling admission is invalidated, and later usage/reconciliation updates for revoked, suspended, expired, or closed allocations fail closed without advancing counters. The 2026-08-20 local readiness snapshot passed coturn REST allocation/quota and production peer-ACL scripts at commit `18a6ea70d0fbf6bc187f5a7242424ad3e88cf5ee`. Local scripts now list active coturn allocations through the loopback admin CLI, submit structured snapshots to Authority, and invoke an idempotent coturn cancel-session executor for unauthorized/conflicting source allocations. This is still a local contract, not proof of scheduled production operation, provider billing reconciliation, multi-node collection, or real active-allocation termination in a public deployment |
...
- macOS M150 loopback, direct signaling and forced local coturn transport E2E
  ...
  commit `18a6ea70d0fbf6bc187f5a7242424ad3e88cf5ee`. The new device
