...
| Relay control/data plane | `services/relay/`, `deploy/phase3/` | Short-term credential control plane and pinned coturn Compose data plane exist. New credentials and usage events are rejected after a persisted revoke. Revocation also persists an idempotent termination outbox and requires a separately deployed trusted HTTPS executor to acknowledge active-allocation termination; the repository does not contain a coturn-specific executor. Public deployment, authoritative byte usage, proven active-allocation disconnect, and multi-node state remain gates |
| Signaling | `services/signaling/` plus Swift/Kotlin clients | Runnable single-instance service, real-process tests, issuer-only invalidation, scoped same-process role-token rotation, a durable per-device deny list, and relay revocation forwarding exist. Rotation state remains memory-only, and signaling does not publish a host-signed replacement Android lease or provide multi-instance authority |
| Rotation/revocation/replay | Protocol, Go core, platform security and product-session directories | Record replay and old-epoch rejection plus peer-scoped signed local revocation have unit/self-test coverage. Android and macOS retain durable retry state for local secret cleanup; macOS lease issuance now checks the peer revocation state inside the same cross-process admission transaction. The Mac scoped authority transport rejects stale responses, and signaling commits its deny before relay forwarding. App lifecycle integration, host-signed two-party replacement leases, active TURN allocation termination, cross-service retry after signaling restart, rotation interoperability, and real reconnect injection remain gates |
...
- Recovery now fails closed into a fresh-session request instead of sending a
  second offer. Scoped signaling rotation and a strict Mac authority client are
  available as source-level primitives, but the local development UI still
  requires manually supplied authority credentials and epoch. Do not claim
  automatic network-handoff recovery until a
...
- Relay session requests remain authenticated by a trusted control-plane bearer,
  ...
  Admin revocation blocks new credentials and later usage events and drives a
  durable, idempotent termination webhook. A deployment must supply and verify
  the coturn-specific executor before claiming an already-issued allocation was
  terminated.
