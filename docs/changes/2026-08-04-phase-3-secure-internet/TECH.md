...
8. Open the negotiated Protocol v1 DataChannel transport boundary:
   ...
   - `vibescreen.audio.v1`: capability-gated Host-to-Android PCM product path,
...
The audio channel now carries the same Host-to-Android PCM S16LE microphone
contract used by the USB/LAN Protocol v1 path over the protected
`vibescreen.audio.v1` DataChannel. It remains capability-gated and optional:
peers that do not negotiate `CAPABILITY_AUDIO`, managed-policy denial, capture
unavailability, playback-adapter rejection, or capture/playback runtime failure
disable audio without upgrading any release claim. The bulk channel now has
offline macOS/Android product-session wiring for explicit file-transfer
offer/accept, bounded Protocol v1 file chunks over `vibescreen.bulk.v1`,
progress/cancel/complete control messages, SHA-256 validation, and
managed-policy/resource-limit gating. These are contract and local
product-session readiness only: real macOS plus Android public-Internet WebRTC
file-transfer E2E over a deployed TURN route, real Internet audio
capture/playback, bidirectional audio, Mac system-output capture, and Internet
clipboard synchronization remain separate release gates.
...
issuance.
Authority can also issue an admin/operator session profile for already
...
engine/signaling layer can actually perform a restart, and it keeps application
traffic suspended during that window. A detected validated-network handoff is
treated as a session-authority boundary instead of an ICE-restart candidate:
both product peers fail closed directly into fresh-session recovery, invalidate
the old transport owner, ignore late callbacks and old-epoch traffic, and require
the owner to supply a fresh signaling session, role token, TURN credential where
...
Android-local MainActivity profile import and first lease bootstrap now have
2026-09-16 P0110 no-Host evidence using an in-memory instrumentation authority,
but production Authority-issued Android import/bootstrap, device proof of the
complete handoff, and public-network E2E remain release gates.
...
- The current `/v1/usage` byte/session ledger is non-authoritative telemetry;
  it is not a real-time spend boundary until a trusted coturn collector is
  deployed. In `production_authority` mode, relay now requires `allocation_id`
  and asks Authority to admit every non-duplicate usage event before mutating the
  ledger, so revoked, expired, unknown, quota-exceeded, or conflicting
  allocation state fails closed. `scripts/phase3/coturn_reconcile.py` defines the
  expected structured snapshot submission, optional external exporter-command,
  bounded retry loop, and disconnect-executor contract for that collector.
  `scripts/phase3/coturn_cli_control.py` can use relay's allocation registry to
  export or disconnect exact coturn CLI sessions in local/operator runs. Neither
  helper observes coturn by itself as a production daemon or provides a deployed
...
rendezvous. The local lease issuer reserves the Authority-supplied
`session_epoch` in pairing-scoped durable Keychain state and rejects values at or
below the local high-water mark before signing. Packet seal and
...
Authority-backed profile request and refresh are wired. Android-local profile
import/bootstrap now has P0110 no-Host MainActivity evidence using an in-memory
instrumentation authority, but production Authority-issued Android bootstrap,
real ScreenCaptureKit/CGDisplayStream capture,
...
| macOS product session | `baseline/MacHost/Sources/Phase3/ProductSession/`, `AppDelegate.swift`, `AudioCapture.swift`, `SettingsWindow.swift`, stasel WebRTC `150.0.0` | Internet mode is separated from the legacy TCP server; capture/HEVC, optional Host-to-Android PCM microphone capture, Protocol v1 control/media/audio, protected DataChannels, touch injection, direct/forced-TURN selection, Keychain credentials, and actionable state UI are wired. Audio remains capability-gated and disabled when managed policy denies capture, capture is unavailable, the peer rejects `AudioConfig`, or capture start/runtime fails. The 2026-08-20 local readiness snapshot passed synthetic direct/relay product sessions at commit `18a6ea70d0fbf6bc187f5a7242424ad3e88cf5ee`, and the narrower 2026-08-05 Nubia local synthetic-media interop record at commit `597518f948075e396352bc353afcec01a30303f3` still exists for that dated device/source pair only. The current worktree's local product E2E sends real VideoToolbox HEVC keyframe/delta payloads over the production WebRTC media DataChannel to a synthetic Protocol v1 peer, with synthetic pixel-buffer input and no capture server. None of these results prove real ScreenCaptureKit device streaming, Android MediaCodec decode, Android `AudioTrack` playback, visible Mac input effects, public Internet, or handoff, which remain gates |
| Android product session | `MainActivity.kt`, `InternetSessionProfileStore.kt`, Android Internet packages, `InternetAudioPlayback.kt`, `io.github.webrtc-sdk:android:144.7559.09` | Internet UI scans the pairing offer, completes the copied request/acceptance flow, imports a strict host-signed short-lived lease, selects direct/forced TURN, drives Protocol v1 video/touch/audio and decoder state, and exposes connect/disconnect/revoke/error/recovery. The audio path configures PCM S16LE playback only after negotiated `.audio`, a valid monotonic `AudioConfig`, and a playback adapter accept; unsupported capabilities, missing playback, stale audio epochs, malformed packets, or playback rejection fail closed without claiming runtime audio. Lease verification precedes persistence/high-watermark changes; durable session/identity epochs reject stale ciphers and permit monotonic reauthorization after revoke. A fresh-session request or terminal failure invalidates the old transport owner, so late route/connected callbacks cannot restore touch, audio, or heartbeat. Credential, pairing and revocation cleanup retain restart-safe retry state. Tokens and pairing/session secrets are AndroidKeyStore-wrapped; sensitive dialogs disable screenshots/autofill, and release cleartext is disabled. The historical Nubia P0110 run is dated 2026-08-05 and bound only to commit `597518f948075e396352bc353afcec01a30303f3`; it covers local UI, direct/forced-coturn Android↔Mac product interop and application AEAD with synthetic Protocol v1 media. The 2026-09-16 P0110 no-Host run covers Android-local MainActivity profile import/bootstrap through an in-memory instrumentation authority, including retryable rejected-draft recovery, local revoke, and repair/re-pair cleanup. The current Android JVM contract confirms Annex-B HEVC-like Internet media frames are preserved through the Protocol v1 assembler into the decoder callback payload and that Internet audio config/packet/cleanup boundaries are fail-closed, but it does not instantiate MediaCodec or prove audible playback. These are not current-worktree device decoder or audio evidence and must not be extrapolated to later commits, real screen capture, public Internet, rotation, handoff, or soak |
...
| Signaling | `services/signaling/` plus Swift/Kotlin clients and `services/authority/` | Runnable memory and PostgreSQL store backends, explicit local/production-authority modes, fail-closed store readiness, fail-closed authority admission and per-request authorization, authority-issued session adoption after successful role authorization, device-revocation rejection, issuer-only invalidation, restart-durable PostgreSQL routing tests, and real two-process PostgreSQL tests exist. In authority mode, long-poll waits are split into bounded reauthorization windows so revocation wins during a pending poll without waiting for the full client timeout. PostgreSQL-backed routing now has focused cross-instance delivery coverage, connection-scoped long-poll waiter leases, and shared per-device/action session-create limiter rows for instances using the same database. The 2026-08-20 local readiness snapshot passed Phase 3 service verification and local Authority container gating at commit `18a6ea70d0fbf6bc187f5a7242424ad3e88cf5ee`. macOS local/offline Authority-backed session-profile request allocation, invocation, and fresh-session refresh are wired, and the 2026-09-16 P0110 no-Host run covers Android-local MainActivity profile import/bootstrap through an in-memory instrumentation authority. Production Authority-issued Android import/bootstrap, device handoff, and public-network E2E remain open. It remains a one-offer router; multi-instance throughput, public ingress/load-balancer behavior, production coturn exporter/disconnect wiring, active transport disconnect, and multi-region consistency remain gates |
...
| WebRTC bulk product-flow evidence | `tools/vibescreen_evidence/phase3_webrtc_bulk_product_flow.py`, `tools/schemas/phase3-webrtc-bulk-product-flow-gate.schema.json`, [TEST.md](TEST.md) | A dedicated fail-closed child gate now owns the public Internet `vibescreen.bulk.v1` file-transfer product-flow closure boundary. It requires retained real macOS Host and real Android device evidence over a deployed public TURN relay WebRTC route, Authority-bound TURN credential and coturn allocation lease validation, approved bidirectional product file transfer, ordered chunk offsets, per-chunk SHA-256 validation, monotonic progress, final chunk and final file SHA-256 equality, bounded backpressure, timeout cancellation with transfer-slot recovery, cancel/disconnect cleanup, no synthetic peer or plaintext fallback, and AES-256-GCM record-layer separation. It rejects PR #404-style relay DNS/`/readyz`/disk hardening, USB/LAN file transfer, local loopback, forced local coturn, synthetic peers, and raw hook tests as substitutes. Its release checklist must still carry relay production readiness, real capture-to-MediaCodec continuity, handoff, revocation, external-camera latency, two-hour soak, and packet-capture confidentiality; even a child-gate pass cannot directly close Phase 3 release |
| WebRTC audio product-flow evidence | `baseline/MacHost/Sources/Phase3/ProductSession/InternetAudioCapture.swift`, `baseline/AndroidClient/app/src/main/java/dev/telemachus/display/internet/InternetAudioPlayback.kt`, `docs/changes/2026-09-01-internet-audio-datachannel/TEST.md` | The current implementation wires the protected `vibescreen.audio.v1` product path for Host-to-Android PCM S16LE microphone packets and verifies protocol codec, macOS session negotiation/capture-failure/recovery behavior, Android playback-adapter admission, stale-epoch rejection, packet routing, and disconnect cleanup in offline tests. This is offline product-flow evidence only. It does not prove public Internet routing, signed Host + Microphone/TCC runtime behavior, real Android `AudioTrack` output, packet-capture confidentiality, handoff continuity, latency, or soak |
...
- Recovery now attempts a bounded transport-level ICE restart only for ordinary
  disconnect recovery when the underlying engine/signaling path can perform one;
  otherwise, and after bounded exhaustion, it fails closed into a fresh-session
  request. Validated network handoff in the product Internet session now requests
  a fresh session immediately instead of first attempting ICE restart, preventing
  reuse of stale signaling state and application record keys across a route
  boundary. The local development UI still requires manually supplied authority
  credentials and epoch. Do not claim automatic network-handoff recovery until a
  real Mac/Android run proves automatic profile issuance, new signaling tokens,
...
- Relay session requests remain authenticated by a trusted control-plane bearer,
  ...
  trusted snapshot, or call an external exporter command that emits the same
  strict JSON snapshot, and force unauthorized/conflicting/revoked source
  allocations through an idempotent external disconnect executor; missing
  ledger-only allocations still require a deployment policy before closure. The
  coturn CLI control helper gives operators a tested way to map relay allocation
  registry entries to exact coturn sessions and issue loopback CLI disconnect
  commands when the mapping is unambiguous. The repository still does not
  include a real coturn management exporter, production scheduler, or
  production-proven concrete data-plane disconnect executor.
