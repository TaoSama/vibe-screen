...
Base: origin/main at e5d8c6fd5089eaaf1d36eab30a439264de53688b
...
--json number,title,headRefName,headRefOid,baseRefName,updatedAt,isDraft,mergeStateStatus,url | jq 'map(select(.headRefName != "codex/phase0-after805-aggregate-refresh")) | sort_by(.number)'`
...
#805 is now merged into the audited mainline input. PR #737 remains merged into
the audited mainline input as an earlier aggregate refresh. Current
`origin/main` also contains PR #737 through PR #805 after that refresh,
excluding the closed-unmerged PRs listed below. PR #804 is the after-803
aggregate refresh. PR #805 hardens the Android camera-recovery instrumentation
lifecycle and host-side cleanup; because it touched non-aggregate paths, this
refresh binds `source.base_commit` directly to the PR #805 squash merge commit.
Neither PR adds Host-backed product evidence.
...
PR #774, and PR #776 through PR #805 were collected
...
hardening, controller Host readiness invalid-UTF-8 fail-closed hardening,
Android no-Host wireless action plus QR static-contract test hardening, previous
aggregate refresh evidence, no-Host wireless UI test isolation, file-transfer
...
This after-805 refresh extends the audited merged range with PR #737 through PR
#750, PR #752, PR #754 through PR #766, PR #768, PR #770 through PR #772, PR
#774, and PR #776 through PR #805. These additional
...
scanner permission recovery, file-transfer dialog visible-frame coverage, the
PR #803 privacy-hardened aggregate input, the PR #804 after-803 aggregate
refresh, and PR #805 Android camera-recovery instrumentation lifecycle and
cleanup hardening.
...
| Protocol v1 contract and CI gates | pass | none | Current main at `e5d8c6fd5089eaaf1d36eab30a439264de53688b` is bound by this source audit. Previously retained successful CI snapshots remain source/build evidence; the after-805 evidence bundle also records main workflow metadata for this audited source. These CI snapshots are not counted as new Host-backed product evidence. |
| Android tests and debug APK clean build | pass | none | Current main at `e5d8c6fd5089eaaf1d36eab30a439264de53688b` includes earlier source/unit/offline/tooling and no-Host readiness updates plus PR #799 through PR #805, excluding the closed-unmerged PRs listed above. The newly audited inputs include Android Internet camera recovery, profile-import/lease UX evidence, file-transfer dialog coverage, aggregate/privacy refreshes, and Android camera-recovery instrumentation lifecycle/cleanup hardening. These updates are not treated as Host-backed product evidence. |
| macOS release build and full-Xcode unit tests | pass | none | Current main at `e5d8c6fd5089eaaf1d36eab30a439264de53688b` is bound by this source audit. Previously retained successful CI snapshots remain source/build evidence, while local 2026-08-28 full-Xcode readiness remains blocked because that machine had Command Line Tools selected and is not a replacement XCTest pass. |
...
| Android USB stream, reconnect, stale epoch, and codec fallback | pass | none | Closed by retained historical real-device baseline evidence; current-base insufficient attempts remain boundary records and do not claim a fresh USB pass. Merged PR #569 through PR #805 add source/unit/offline, tooling, aggregate, or no-Host readiness evidence, but those records are not counted as stream/reconnect, LAN route, TCP 54321, Host-backed product, physical HID pointer, controller, or retained device evidence. |
...
native-pointer HID observation artifact validation hardening only. PR #729
...
PR #771, PR #774, PR #776 through PR #784, and PR #799 through PR #805 contribute
...
dialog visible-frame coverage only. PR #803 contributes aggregate refresh and
Android serial privacy scanner/test hardening only. PR #804 contributes the
after-803 aggregate refresh only. PR #805 contributes Android camera-recovery
instrumentation lifecycle and cleanup hardening only. PR #795 contributes Android system clipboard
...
`e5d8c6fd5089eaaf1d36eab30a439264de53688b`, filters only this aggregate
refresh branch from the open upstream-input snapshot, records merged PR #569
through PR #805 with explicit closed-unmerged exclusions, validates each
...
`evidence/2026-09-16-after-805-current-main-gate-blocked/`; local verification
