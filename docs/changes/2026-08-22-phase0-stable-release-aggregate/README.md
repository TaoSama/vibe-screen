...
Base: origin/main at eb87c4965a8db6552af5a0901f72666129ab16e6
...
returned no open PRs. PR #652 and PR #653 are now merged into the audited
mainline input after the previous aggregate refresh. Merged PR #569 through PR
#575, PR #577 through PR #626, and PR #628 through PR #653
...
audited input. PR #568, PR #576, PR #627, and PR #646 were closed without
merging, so they are explicitly excluded from the audited merged range. Merged
PR #569 through PR #575, PR #577 through PR #626, and PR #628 through PR #653
...
peripheral input diagnostics and pointer guards, the previous aggregate refresh,
and Android Internet secondary-action large-text no-Host UI/UX evidence.
...
| Protocol v1 contract and CI gates | pass | none | Current main at `eb87c4965a8db6552af5a0901f72666129ab16e6` is bound by this source audit. The retained push workflow snapshot records Phase 0 checks run `34070930516` completed successfully for protocol, evidence-tools, phase3, Android, and macOS jobs. |
| Android tests and debug APK clean build | pass | none | Current main at `eb87c4965a8db6552af5a0901f72666129ab16e6` includes PR #640 peripheral input payload snapshotting, PR #642 Host setup guidance classification, PR #641 Android audio jitter gap recovery, PR #644 Android input dispatch boundary validation, PR #647 evidence fail-closed validation hardening, PR #645 PCM audio protocol boundary hardening, PR #650 clipboard/file-transfer evidence gate hardening, PR #651 peripheral input diagnostics/pointer guards, PR #652 aggregate refresh evidence, and PR #653 Android Internet secondary-action large-text no-Host UI/UX evidence. These are source/unit/offline/tooling/no-Host readiness updates and are not treated as Host-backed product evidence; the retained Phase 0 checks run `34070930516` completed the Android job successfully. |
| macOS release build and full-Xcode unit tests | pass | none | Current main at `eb87c4965a8db6552af5a0901f72666129ab16e6` is bound by this source audit. The retained Phase 0 checks run `34070930516` completed the macOS job successfully. Local 2026-08-28 full-Xcode readiness remains blocked because that machine had Command Line Tools selected, so it is not a replacement XCTest pass. |
| macOS Host hardware compatibility matrix | open | none | Published current-base `macos-hardware-compatibility-gate` summaries exist for Mac16,8 readiness, but they are `blocked`. PR #546 strengthened the preflight so Host readiness now fails closed without read-only Screen Recording, Accessibility, and Microphone TCC rows bound by `csreq` to the stable signing requirement, bundle id, install path, and source provenance. Merged PR #625 tightens Host TCC identity preflight only, and closed-unmerged PR #627 does not provide passing hardware compatibility matrix evidence. A real packaged Host launch, Protocol v1 stream, input, reconnect evidence, Intel Macs, additional Apple silicon models, macOS builds, and display topologies still need exact-row passing evidence. |
| Android USB stream, reconnect, stale epoch, and codec fallback | pass | none | Closed by retained historical real-device baseline evidence; current-base insufficient attempts remain boundary records and do not claim a fresh USB pass. Merged PR #569 through PR #575, PR #577 through PR #626, and PR #628 through PR #653 add Android no-Host UI, keyboard, clipboard, audio, AV1, controller, managed-policy, file-transfer, aggregate evidence, host-unreachable/network-down/Host setup guidance, P0110 no-Host UI/UX review, telemetry/Host RSS gate hardening, Host TCC identity preflight, phase0 guidance, input coalescing, peripheral payload snapshotting, audio jitter recovery, Android input dispatch boundary validation, evidence-tool fail-closed hardening, aggregate refresh evidence, PCM audio protocol boundary hardening, clipboard/file-transfer evidence gate hardening, peripheral input diagnostics/pointer guards, and Android Internet secondary-action large-text no-Host UI/UX evidence, but those records are not counted as stream/reconnect, LAN route, TCP 54321, Host-backed product, physical HID pointer, controller, or retained device evidence. |
| Telemetry and external latency artifact archive | insufficient | none | PR #557 adds Android stream telemetry counters for dropped frames, decoder latency, session epoch, wire mode, and heartbeat source with focused JVM coverage, but those counters are diagnostic/source readiness only. PR #545 tightened the latency evidence gate, and raw telemetry plus the latest current-base latency preflight remain insufficient; no external-camera latency sample package, raw camera media, or synchronized-clock physical-input proof is archived for this aggregate. The aggregate checker now also fails closed if this gate is marked `pass` without repo-local structured JSON evidence containing both a passing formal `latency_evidence_gate` report and a passing `android_usb_live_smoke` report with stream telemetry and decoder counters. Former tooling PR references are merged or stale baselines, not active open owners. |
...
PR #650 is included in this refresh as evidence-tool and gate-contract hardening
only. It tightens the clipboard and file-transfer product E2E validators so
missing, empty, reused, absolute, parent-directory, or symlink-escaped retained
artifacts fail closed; it also requires exact clipboard artifact roles and
bidirectional file-transfer endpoint/session/progress/ID/name/SHA-256
separation. PR #651 is included as Android peripheral-input diagnostics, pointer
guard, and focused source/offline coverage only. PR #652 is included only as the
previous aggregate refresh evidence. PR #653 is included only as Android
no-Host UI/UX evidence for Internet secondary actions under large text; it does
not start or validate a Host session and does not prove Host/TCC readiness, RSS
behavior, native pointer HID, controller runtime, clipboard product E2E,
file-transfer product E2E, or external latency. These updates do not convert any
...
The 2026-09-07 UTC / 2026-09-07 local manifest refresh binds the aggregate
source guard to `eb87c4965a8db6552af5a0901f72666129ab16e6`, records no open
PRs at query time, records merged PR #569 through PR #575, PR #577 through PR
#626, and PR #628 through PR #653 as `main`-targeted
...
through PR #653 range because they are not merged in the audited mainline base.
The retained current refresh summary
bundle is under `evidence/2026-09-07-after-653-current-main-gate-blocked/`; local
