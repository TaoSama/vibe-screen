# P0110 current-base reconnect timing blocked readiness

This directory records a current-base blocked owner refresh for the Phase 1
...
- Current source commit: `87e16d8bea4446c1ca449045678f1bafc7fd6cb2`
...
device/product `pacific`, Android `16`, and SDK `36`. `adb-reverse-before.txt`
records the existing USB reverse mapping `UsbFfs tcp:54321 tcp:54321`, and
...
The real reconnect timing attempts were blocked before disruption because the
source-bound stable Host prerequisites were not satisfied:
...
- TCC permissions were not inspected after Host bundle signing inspection
  failed.
- The trusted-LAN preflight also remains blocked: Android Wi-Fi is not
  associated, `wlan0` has no IPv4 address or route to the Mac LAN candidate,
  and no LAN stream, secure-record negotiation, or reconnect was observed.
...
commands were `pgrep -x sfltool`, recorded in `sfltool-start.txt` and
`sfltool-end.txt`.

## Summary

`reconnect-timing-summary.json` was generated with the `reconnect_timing`
blocked path and records:
...
Focused verifier coverage should be run from the repository root:
