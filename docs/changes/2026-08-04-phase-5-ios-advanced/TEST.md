...
`stream_id` values, rejects stale lower epochs, and releases routes when a
session closes. The session coordinator now validates touch, pointer, scroll,
keyboard, controller, and host-action targets against the route owned by the
same client session before any input action reaches the Host injection layer.
