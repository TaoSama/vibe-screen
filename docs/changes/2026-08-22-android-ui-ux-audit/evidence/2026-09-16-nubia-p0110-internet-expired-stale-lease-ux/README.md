...
| Focused P0110 instrumentation | `android-test-results/debug/TEST-P0110 - 16-_app-.xml`, `logs/connected-expired-stale-lease-ux-instrumentation.log` | 1/1 passed, failures=0, errors=0, skipped=0; Gradle logged `BUILD SUCCESSFUL in 28s` |
...
| Secondary actions | per-test logcat PASS marker | `scan_import_revoke_available=true`; the test checks Connect is visibly disabled and Scan, Import, and Revoke remain visible and enabled |
...
- The stale state is produced by the product session coordinator requiring a lease newer than epoch 1 after a simulated session-start failure; the expired state is produced by importing a short-lived, validly signed lease, waiting for its signed expiry time, and then foregrounding/recreating `MainActivity`.
- `uninstall-app.txt` and `uninstall-test.txt` report successful uninstall after Gradle cleanup; final package listing is empty and final Camera appops has no UID, so no installed `dev.telemachus.display` package remained for user 0.
