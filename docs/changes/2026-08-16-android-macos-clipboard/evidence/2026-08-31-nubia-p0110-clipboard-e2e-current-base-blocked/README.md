...
Date: 2026-08-31 (local, Asia/Shanghai; UTC 2026-08-31T04:50Z)
Source base: `origin/main` at `075dc157c`
Branch: `codex/clipboard-e2e-p0110-owner-20260831b`
...
- The run started from `origin/main` HEAD `075dc157c` in the dedicated
  `codex/clipboard-e2e-p0110-owner-20260831b` worktree.
- The Host readiness command did not opt into the login-item diagnostic and did
  not run `/usr/bin/sfltool dumpbtm`. `pgrep -x sfltool` observed no retained
  `sfltool` process before or after collection.
- The target device matched nubia P0110 / pacific / Android 16 / API 36. ADB
  reverse for TCP `54321` was present and the Android app was foreground.
- Current-source Android debug and androidTest APKs built and installed on the
  device.
- Android focused clipboard JVM tests passed as part of `testDebugUnitTest`.
...
- USB preflight is blocked because the Mac Host is not listening on TCP
  ...
  smoke could be admitted, even though ADB reverse was present and the Android
  app was foreground.
...
- `source-baseline.txt`, `git-head.txt`, `git-origin-main.txt` - branch and
  HEAD snapshots.
- `pgrep-sfltool-start.txt` / `.exit` - startup safety check output showing no
...
- `android-clipboard-instrumentation.txt` - local Android
