...
- `git rev-parse origin/main` resolved to
  `f669dc3521c97364533653016747bc1cb4f8d015`, the PR #548 merge commit.
- `gh pr view 546`, `gh pr view 547`, and `gh pr view 548` confirmed those
  PRs are merged into the audited base.
...
## 2026-09-04 UTC / 2026-09-04 local refresh

- The current base is the PR #548 merge commit
  `f669dc3521c97364533653016747bc1cb4f8d015`.
- PR #546, PR #547, and PR #548 are merged, and there were no open PRs when
  this refresh branch captured the snapshot. Previous owner mappings from #157
  through #548 are now merged, closed, or superseded historical context rather
  than active external gate owners.
- PR #546 tightened macOS Host permission-preflight fail-closed behavior and
  read-only TCC identity binding, but it did not produce a stable-signed,
  source-bound, TCC-authorized Host compatibility row.
- PR #547/#548 added Android transfer-approval and no-Host retry-card UX
  coverage, including a P0110 no-Host smoke record. These are Android-side
  source/UI readiness records and do not close bidirectional macOS product E2E
  or Host-dependent runtime gates.
...
| macOS Host hardware compatibility matrix | `README.md:32`, `README.md:270-287` | Published current-base Mac16,8 summaries are blocked readiness records; PR #546 strengthens the fail-closed permission/TCC preflight used before future row collection. | none | No. | No exact-row pass with stable signing/TCC, source-bound Host provenance, full macOS checks, packaged runtime launch, Protocol v1 stream, input smoke, and reconnect evidence. | Collect exact-row passing evidence with the macOS compatibility runbook; do not generalize from local readiness. |
| Telemetry and external latency archive | `README.md:410-424` | Latest current-base latency-camera preflight is blocked/insufficient readiness. | none | P0110 can be the Android screen/input target. | No raw external-camera sample package or synchronized-clock physical-input proof exists for any required profile. | Collect raw camera/samples/manifest or synchronized-clock physical-input evidence, then run the latency evidence gates. |
