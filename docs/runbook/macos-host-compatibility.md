...
| --- | --- | --- | --- |
| Apple silicon | Local Apple silicon development and historical device evidence exist, and blocked `macos-hardware-compatibility-gate` summaries are published for Mac16,8 current-base readiness. The latest current-base blocked record is [2026-08-31-macos-host-compatibility-current-base-blocked](../changes/2026-08-21-host-signing-tcc-preflight/evidence/2026-08-31-macos-host-compatibility-current-base-blocked/README.md). No passing row exists. | Apple silicon has been locally exercised. Do not claim a compatibility row until a gate summary passes for the exact host. | Collect one row per Mac model family, macOS build, display topology, and transport. |
