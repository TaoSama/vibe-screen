...
including a 2026-09-05 refresh that expands the smoke to 5 tests for ordinary
text, instrumentation-argument round trips, Unicode/256 KiB foreground text,
and non-text `ClipData`, and hardens the gate against zero-test or
summary-only instrumentation logs. The gate still requires retained product
evidence for exact Android ClipboardManager and macOS NSPasteboard endpoints,
verified session epoch, change ID, SHA-256 digest, bounded byte length, and
distinct markers for both transfer directions. The real Android ClipboardManager
