...
The local product E2E now strengthens its media payload from fixed synthetic
strings to real VideoToolbox-generated HEVC keyframe and delta frames over the
production WebRTC media DataChannel, still with a synthetic Protocol v1 peer and
synthetic pixel-buffer input. That check does not start ScreenCaptureKit or
CGDisplayStream and is not Android MediaCodec, visible UI, or public-Internet
evidence.
