...
| macOS Host compatibility | Compatibility matrix gate is open. Apple silicon has local exercise, historical device evidence, and published blocked `macos-hardware-compatibility-gate` summaries for Mac16,8 current-base readiness, with the latest current-base refresh under [2026-08-31-macos-host-compatibility-current-base-codex-task-blocked](docs/changes/2026-08-21-host-signing-tcc-preflight/evidence/2026-08-31-macos-host-compatibility-current-base-codex-task-blocked/README.md); no passing row exists because stable signing plus Screen Recording, Accessibility, and Microphone TCC, source-bound Host provenance, full macOS checks, and same-session runtime stream/input/reconnect evidence are missing. Intel Macs, additional Apple silicon models, macOS builds, and display topologies still need exact-row passing evidence before support claims |
| USB transport | ADB reverse on TCP port `54321`; real-device stream verified |
| Video | ScreenCaptureKit/CGDisplayStream, VideoToolbox HEVC/H.264 encoding, and Android MediaCodec HEVC/H.264 decode. AV1 is a later-phase/backlog codec, not a current Host/device stream codec: Protocol v1 only reserves CODEC_AV1 and the macOS/Android/iOS runtime-admission foundation is in place, but the current Host does not advertise AV1, Android does not offer AV1 in product sessions, iOS does not advertise AV1 decode capability, and no AV1 real-stream Host/device acceptance is recorded; see the [AV1 codec capability gate](docs/changes/2026-08-21-av1-codec-capability/TEST.md) record |
| Audio | Protocol v1 USB/LAN now wires a capability-gated PCM S16LE microphone-capture path from the macOS Host to Android `AudioTrack`, with offline Host/Android protocol, packetization, playback, and LAN secure-record tests. Internet mode now also has Protocol v1 Host-to-Android PCM microphone DataChannel wiring over protected `vibescreen.audio.v1`, with offline coverage for macOS capture-adapter boundaries, Android playback-adapter admission, packet routing, fail-closed rejection, recovery, and disconnect cleanup. The [2026-09-08 Nubia P0110/pacific current-main no-Host smoke](docs/changes/2026-08-24-p0110-audio-current-base/evidence/2026-09-08-p0110-audio-android-track-no-host-current-main/README.md) verifies the Android playback adapter can start, write one synthetic PCM packet through `AudioTrack`, and close cleanly. The [2026-09-15 P0110 Internet audio readiness no-Host smoke](docs/changes/2026-08-24-p0110-audio-current-base/evidence/2026-09-15-p0110-internet-audio-readiness-no-host/README.md) additionally verifies that Settings selects the Internet playback snapshot, renders the active PCM format and accepted/written counters, and clears stale readiness after disconnect. These checks are Android-local and do not prove real Android/macOS audio E2E. The 2026-08-30 Nubia P0110/pacific current-base refresh remains blocked because the installed Host is not a current-source stable-signed Microphone/TCC-ready bundle, no Host listener was observed, and retained logs show no `CAPABILITY_AUDIO`, accepted `AudioConfig`, channel `3`, Host-origin audio packets, playback confirmation, or disconnect cleanup evidence. Real public-Internet audio playback evidence remains open |
...
| Clipboard | Protocol v1 text clipboard forwarding is wired for Android <-> macOS with explicit send/get/overwrite actions, strict UTF-8 `text/plain`, SHA-256/origin/session-epoch validation, deny-wins managed-policy gating, and a 1 MiB negotiated size ceiling. Android JVM tests, Protocol v1 fixtures, and the Mac Protocol v1 self-test pass on current main. The [2026-09-08 Nubia P0110 current-main no-Host Android clipboard baseline](docs/changes/2026-08-16-android-macos-clipboard/evidence/2026-09-08-nubia-p0110-clipboard-android-baseline-current-main/README.md) expands local Android ClipboardManager smoke to 8 tests, including Unicode/256 KiB and 320 KiB foreground text, empty-clipboard clearing, non-text `ClipData`, and first-item non-text multi-item handling; the fail-closed gate now requires that complete 8-test current-run smoke instead of older 5-test logs. The latest [2026-09-14 Nubia P0110 after-782 current-main no-Host UI/UX refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-14-nubia-p0110-no-host-uiux-after-782-current-main/README.md) retains focused clipboard confirmation layout coverage for send, receive, overwrite, large text, narrow portrait, and pending-status controls without a product Host session, within a 135/135 Android instrumentation run plus 129/129 focused JVM checks that keep no-Host refresh paths away from Android ClipboardManager read/write boundaries and no `tcp:54321` reverse mapping. Real Android ClipboardManager <-> macOS NSPasteboard USB/LAN E2E remains open because Host readiness and real transport prerequisites are blocked and no bidirectional product transfer record exists |
| File transfer | Protocol v1 includes a bounded single-file transfer path for Android/macOS USB/LAN sessions with explicit sender action, receiver approval, safe basenames, negotiated limits, chunk ordering, session-epoch checks, SHA-256 verification, deny-wins policy handling, and cancel/disconnect cleanup covered by offline JVM/protocol tests. Android source and no-Host device smoke now cover control-bar transfer UI readiness, including incoming progress/cancel visibility and outgoing-progress controls, plus a [2026-09-05 Nubia P0110 no-Host UI/UX refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-05-nubia-p0110-no-host-uiux-current-source/README.md) for the disconnected, retry/error, Settings readiness, LAN, Internet, and landscape large-text surfaces. The [2026-09-14 Nubia P0110 after-782 current-main no-Host UI/UX refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-14-nubia-p0110-no-host-uiux-after-782-current-main/README.md) reruns the broader focused no-Host layout instrumentation with 135/135 passing tests and 129/129 focused JVM checks. The dedicated [2026-09-15 Nubia P0110 no-Host file-transfer UI smoke](docs/changes/2026-08-21-file-transfer-e2e/evidence/2026-09-15-nubia-p0110-no-host-file-transfer-smoke-after-785/README.md) verifies all 5 required control-bar and offer/preflight dialog methods. The [P0110 Android-local MediaStore Downloads runtime smoke](docs/changes/2026-08-21-file-transfer-e2e/evidence/2026-09-15-nubia-p0110-no-host-mediastore-downloads-runtime/README.md) verifies production-path incoming publication, exact bytes/SHA-256 readback, `IS_PENDING=0`, and failed-entry deletion. The [P0110 Android-local outgoing staging runtime smoke](docs/changes/2026-08-21-file-transfer-e2e/evidence/2026-09-15-nubia-p0110-no-host-outgoing-staging-runtime/README.md) verifies production `ContentResolver` reads from a real `content://` source into app-private staging with exact bytes, length, MIME type, SHA-256, and oversize cleanup; focused JVM coverage separately binds those staged bytes to offer metadata, ordered first/final chunks, and explicit cancel/disconnect cleanup ownership. The latest [P0110 Android system-share target and pending-draft smoke](docs/changes/2026-08-21-file-transfer-e2e/evidence/2026-09-15-nubia-p0110-no-host-system-share-target/README.md) verifies Vibe Screen appears as a single-file share target, cold-launches into a persistent cancellable pending draft without reading the source URI, survives Activity recreation, and handles a repeated share of the same URI as a new user action. The [P0110 after-797 ACTION_SEND_MULTIPLE single-file smoke](docs/changes/2026-08-21-file-transfer-e2e/evidence/2026-09-15-nubia-p0110-no-host-send-multiple-single-file-after-797/README.md) verifies current source also admits `ACTION_SEND_MULTIPLE` only for exactly one `content://` item and rejects true multi-item shares before provider reads or staging. The [2026-09-17 Nubia P0110 incoming-save recovery run](docs/changes/2026-08-21-file-transfer-e2e/evidence/2026-09-17-nubia-p0110-incoming-save-recovery/README.md) verifies durable adoption before accepted completion, retry/discard UI across Activity recreation and process force-stop, exact-byte MediaStore publication, pending-URI reuse, and one visible Downloads row after cleanup retry. Review stays hidden until a file-transfer-capable session exists and never auto-stages or auto-offers the file. These no-Host checks do not prove Host-backed bytes landing. The dedicated fail-closed Phase 0 gate requires same-session bidirectional evidence with retained source/destination bytes, shared session ID, ordered chunk offsets with final markers, and cancel/disconnect cleanup |
...
2. Grant the macOS host Screen Recording permission. Grant Accessibility if
   touch control is required. Grant Microphone if audio forwarding or a
   compatibility row is required.
...
[docs/changes/2026-08-10-host-rss-growth/evidence/2026-08-31-current-base-host-rss-failclosed-readiness](docs/changes/2026-08-10-host-rss-growth/evidence/2026-08-31-current-base-host-rss-failclosed-readiness/README.md)
and the prior
...
The latest checked-in Phase 0 aggregate refresh after PR #805 remains blocked in
[2026-09-16-after-805-current-main-gate-blocked](docs/changes/2026-08-22-phase0-stable-release-aggregate/evidence/2026-09-16-after-805-current-main-gate-blocked/README.md).
...
environment. The latest Nubia P0110 current-main records preserve a blocked
`clipboard-e2e-gate` verdict with local Android ClipboardManager smoke passing,
including a 2026-09-08 refresh that expands the smoke to 8 tests for ordinary
...
[2026-09-08-nubia-p0110-clipboard-android-baseline-current-main](docs/changes/2026-08-16-android-macos-clipboard/evidence/2026-09-08-nubia-p0110-clipboard-android-baseline-current-main/README.md),
[2026-09-05-nubia-p0110-clipboard-android-baseline-blocked](docs/changes/2026-08-16-android-macos-clipboard/evidence/2026-09-05-nubia-p0110-clipboard-android-baseline-blocked/README.md),
...
- Protocol v1 USB/LAN now has an offline-tested, capability-gated PCM S16LE
  microphone-capture path from macOS Host to Android playback. A 2026-09-06
  Nubia P0110/pacific no-Host instrumentation smoke proves the Android playback
  adapter can create/start/write/close a real `AudioTrack` with one synthetic
  PCM packet; real-device USB and trusted-LAN audio playback evidence remains
  open. The 2026-08-30 Nubia
  ...
  Host-origin audio packets, playback confirmation, or disconnect cleanup.
...
- Harden window migration/return, disconnect recovery, automatic reconnect,
  ...
  is still not run, until each state has retained evidence. The latest
  [no-Host P0110 after-782 current-main UI/UX record](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-14-nubia-p0110-no-host-uiux-after-782-current-main/README.md)
  retains device instrumentation coverage for the Android disconnected, error,
  dialog, settings, control-surface, clipboard, file-transfer, and
  accessibility surfaces, plus focused JVM guardrails that keep no-Host
  refresh paths away from system clipboard and file-transfer side-effect
  boundaries. It is not Host-backed E2E evidence and does not close the Phase 1
  gate.
...
- The macOS Host and Android client now share a transport-neutral, bounded
  ...
  Android keeps incoming and outgoing transfer workflows mutually exclusive and
  surfaces incoming progress and cancellation through the shared control bar in
  focused source/unit coverage. Both sides enforce safe basenames, deny-wins
  managed policy and resource limits, ordered chunks, per-chunk and final
  SHA-256, session-epoch checks, same-session evidence binding,
  ...
  control-bar and dialog UI paths, including the broader
  [2026-09-14 P0110 no-Host after-782 current-main UI/UX refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-14-nubia-p0110-no-host-uiux-after-782-current-main/README.md)
  and the dedicated [2026-09-15 P0110 no-Host file-transfer smoke](docs/changes/2026-08-21-file-transfer-e2e/evidence/2026-09-15-nubia-p0110-no-host-file-transfer-smoke-after-785/README.md),
  plus the [2026-09-15 P0110 Android system-share target smoke](docs/changes/2026-08-21-file-transfer-e2e/evidence/2026-09-15-nubia-p0110-no-host-system-share-target/README.md),
  and the [2026-09-15 P0110 ACTION_SEND_MULTIPLE single-file smoke](docs/changes/2026-08-21-file-transfer-e2e/evidence/2026-09-15-nubia-p0110-no-host-send-multiple-single-file-after-797/README.md),
  not real bidirectional file transfer with retained source/destination bytes,
  shared session ID, ordered chunk-offset artifacts, or disconnect cleanup; no Android real-device file-transfer
  acceptance, public-Internet run, or WebRTC bulk DataChannel path is claimed.
  ...
  child gate and the public-Internet WebRTC bulk child gate. The Phase 0 local
  product gate is the Android USB/LAN child; the WebRTC bulk child belongs to the
  Phase 3 public-Internet boundary. The aggregate consumes existing child gate
  reports only, rejects missing or wrong-kind child reports, never runs device or
  Host collectors, never closes the separate clipboard gate, and treats Nubia
  P0110 evidence as USB/LAN-only unless a real Internet WebRTC bulk product-flow
...
Host build/signing and Screen Recording/Accessibility/Microphone TCC state, a
real Protocol v1 stream, display selection, physical/current-main capture,
virtual-display or fallback behavior, mirror or
