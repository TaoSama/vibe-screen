...
| Input (keyboard/mouse/peripheral) | Touch, touch-derived pointer, keyboard, and mouse-wheel scroll forwarding to macOS CGEvent verified on device; native mouse pointer move/click is wired end to end but pending a physical-HID-mouse confirmation. Protocol v1 stylus pressure, signed two-axis tilt, eraser, two barrel buttons, and hover are independently capability-gated across USB, LAN, and Internet, with old-peer touch fallback and mixed finger/stylus routing. Protocol v1 standard controller lifecycle and full-state forwarding are source-implemented and offline-tested across USB, LAN, and Internet; Android capture and the Host are both bounded to four concurrent controllers, but the path is not verified with multiple physical controllers. The macOS Host advertises controller support only for identity-signed builds that carry the `com.apple.developer.hid.virtual.device` entitlement and pass a runtime virtual-HID probe. Apple entitlement approval, real virtual-HID device creation, physical controller recognition, and end-to-end controller input remain unverified. Physical-stylus drawing-app confirmation and other peripherals also remain open |
...
- CGEvent and Accessibility provide the macOS input adapter. Protocol v1 wires
  touch-derived gestures, keyboard, native pointer/scroll, pen and eraser
  pressure/tilt, barrel buttons, hover/proximity state, and standard controller
  snapshots. Controller snapshots use an `IOHIDUserDevice` virtual gamepad only
  after the signed entitlement and runtime probes succeed. Unsigned or ad-hoc
  builds, builds without the approved entitlement, and builds that fail the
  runtime probe do not advertise that capability. Physical HID mouse, stylus,
  and controller behavior still require their respective device confirmations.
