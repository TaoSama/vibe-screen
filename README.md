...
| Clipboard | Protocol v1 text clipboard forwarding is wired for Android <-> macOS with explicit send/get/overwrite actions, strict UTF-8 `text/plain`, SHA-256/origin/session-epoch validation, deny-wins managed-policy gating, and a 1 MiB negotiated size ceiling. Android JVM tests, Protocol v1 fixtures, and the Mac Protocol v1 self-test pass on current main. The 2026-09-05 Nubia P0110 current-base refresh expands local Android ClipboardManager smoke to 5 tests, including Unicode/256 KiB foreground text and non-text `ClipData`, and hardens the fail-closed gate so zero-test or summary-only instrumentation logs cannot pass. A [2026-09-07 Nubia P0110 current-main no-Host dialog refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-07-nubia-p0110-no-host-dialog-refresh-current-main/README.md) adds focused clipboard confirmation layout coverage for send, receive, overwrite, large text, narrow portrait, and landscape without a product Host session. Real Android ClipboardManager <-> macOS NSPasteboard USB/LAN E2E remains open because Host readiness and real transport prerequisites are blocked and no bidirectional product transfer record exists |
| File transfer | Protocol v1 includes a bounded single-file transfer path for Android/macOS USB/LAN sessions with explicit sender action, receiver approval, safe basenames, negotiated limits, chunk ordering, session-epoch checks, SHA-256 verification, deny-wins policy handling, and cancel/disconnect cleanup covered by offline JVM/protocol tests. Android source and no-Host device smoke now cover control-bar transfer UI readiness, including incoming progress/cancel visibility and outgoing-progress controls, plus a [2026-09-05 Nubia P0110 no-Host UI/UX refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-05-nubia-p0110-no-host-uiux-current-source/README.md) for the disconnected, retry/error, Settings readiness, LAN, Internet, and landscape large-text surfaces. A [2026-09-06 Nubia P0110 current-main expanded UI/layout refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-06-nubia-p0110-no-host-uiux-expanded-current-main/README.md) reran the focused no-Host layout instrumentation on current `origin/main` with 48/48 passing tests, adding Internet pairing/profile-import dialog checks, and no `tcp:54321` reverse mapping. A [2026-09-07 Nubia P0110 no-Host file-offer dialog refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-07-nubia-p0110-no-host-file-offer-dialog/README.md) adds structured incoming offer content coverage for long filenames, large text, narrow portrait, and landscape without a product Host session; a [2026-09-07 current-main no-Host dialog refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-07-nubia-p0110-no-host-dialog-refresh-current-main/README.md) reruns incoming offer plus outgoing preflight dialog coverage together with clipboard confirmations and no `tcp:54321` reverse mapping. These no-Host UI checks do not prove Host-backed bytes landing. A dedicated fail-closed Phase 0 gate tracks real Android/macOS product E2E readiness; the 2026-08-29 Nubia P0110 current-base record keeps it blocked because Host readiness is not satisfied and no bidirectional product transfer evidence exists |
