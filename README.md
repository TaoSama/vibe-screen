...
| Clipboard | Protocol v1 text clipboard forwarding is wired for Android <-> macOS with explicit send/get/overwrite actions, strict UTF-8 `text/plain`, SHA-256/origin/session-epoch validation, deny-wins managed-policy gating, and a 1 MiB negotiated size ceiling. Android JVM tests, Protocol v1 fixtures, and the Mac Protocol v1 self-test pass on current main. The [2026-09-08 Nubia P0110 current-main no-Host Android clipboard baseline](docs/changes/2026-08-16-android-macos-clipboard/evidence/2026-09-08-nubia-p0110-clipboard-android-baseline-current-main/README.md) expands local Android ClipboardManager smoke to 8 tests, including Unicode/256 KiB and 320 KiB foreground text, empty-clipboard clearing, non-text `ClipData`, and first-item non-text multi-item handling; the fail-closed gate now requires that complete 8-test current-run smoke instead of older 5-test logs. The latest [2026-09-10 Nubia P0110 after-731 current-main no-Host UI/UX refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-10-nubia-p0110-no-host-uiux-after-731-current-main/README.md) retains focused clipboard confirmation layout coverage for send, receive, overwrite, large text, narrow portrait, and pending-status controls without a product Host session, within an 87/87 Android instrumentation run plus 19/19 focused JVM control-surface checks that keep no-Host refresh paths away from Android ClipboardManager read/write boundaries and no `tcp:54321` reverse mapping. Real Android ClipboardManager <-> macOS NSPasteboard USB/LAN E2E remains open because Host readiness and real transport prerequisites are blocked and no bidirectional product transfer record exists |
| File transfer | Protocol v1 includes a bounded single-file transfer path for Android/macOS USB/LAN sessions with explicit sender action, receiver approval, safe basenames, negotiated limits, chunk ordering, session-epoch checks, SHA-256 verification, deny-wins policy handling, and cancel/disconnect cleanup covered by offline JVM/protocol tests. Android source and no-Host device smoke now cover control-bar transfer UI readiness, including incoming progress/cancel visibility and outgoing-progress controls, plus a [2026-09-05 Nubia P0110 no-Host UI/UX refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-05-nubia-p0110-no-host-uiux-current-source/README.md) for the disconnected, retry/error, Settings readiness, LAN, Internet, and landscape large-text surfaces. The latest [2026-09-10 Nubia P0110 after-731 current-main no-Host UI/UX refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-10-nubia-p0110-no-host-uiux-after-731-current-main/README.md) reruns the focused no-Host layout instrumentation with 87/87 passing tests, including incoming offer and outgoing preflight dialogs together with clipboard confirmations, Internet pairing/profile-import dialogs, QR scanner, accessibility, gesture preferences, and focused JVM checks verifying no-Host control-surface refresh does not launch the file picker, read source files, publish Downloads, or send file offers. These no-Host UI checks do not prove Host-backed bytes landing. A dedicated fail-closed Phase 0 gate tracks real Android/macOS product E2E readiness and now requires same-session bidirectional evidence with retained source/destination bytes, shared session ID, ordered chunk offsets with final markers, and cancel/disconnect cleanup; the 2026-08-29 Nubia P0110 current-base record keeps it blocked because Host readiness is not satisfied and no bidirectional product transfer evidence exists |
...
- Harden window migration/return, disconnect recovery, automatic reconnect,
  ...
  [no-Host P0110 after-731 current-main UI/UX record](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-10-nubia-p0110-no-host-uiux-after-731-current-main/README.md)
...
- The macOS Host and Android client now share a transport-neutral, bounded
  ...
  control-bar and dialog UI paths, including the latest
  [2026-09-10 P0110 no-Host after-731 current-main UI/UX refresh](docs/changes/2026-08-22-android-ui-ux-audit/evidence/2026-09-10-nubia-p0110-no-host-uiux-after-731-current-main/README.md),
