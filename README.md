...
| Audio | Protocol v1 USB/LAN now wires a capability-gated PCM S16LE microphone-capture path from the macOS Host to Android `AudioTrack`, with offline Host/Android protocol, packetization, playback, and LAN secure-record tests. This is not system-output capture. The 2026-08-28 Nubia P0110/pacific current-base record remains blocked because the installed Host is not a current-source audio-capable, stable-signed, Microphone/TCC-ready bundle and no `CAPABILITY_AUDIO`, `AudioConfig`, channel `3`, `AudioTrack` write, or playback-confirmation evidence was collected |
...
- Protocol v1 USB/LAN now has an offline-tested, capability-gated PCM S16LE
  ...
  P0110/pacific current-base owner record is blocked before audio negotiation:
  USB video and a Host listener were observed, but the installed Host bundle
  lacks current-source provenance and Microphone/TCC readiness, and no
  `CAPABILITY_AUDIO`, accepted `AudioConfig`, channel `3` audio packets,
  Android `AudioTrack` writes, or playback confirmation were retained.
