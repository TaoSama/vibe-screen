...
- Native ArkTS/ArkUI source and an independent Protocol v1 product-session core
  ...
  legacy plaintext fallback marking. ArkUI now wires TCP,
  XComponent, AVCodec, Asset Store, foreground suspension, and bounded reconnect in
  source. The production Harmony TCP path still uses the explicit plaintext
  Protocol v1 upgrade and must not be reported as encrypted until the HUKS-backed
  provider and transport integration pass DevEco, Host, and MatePad gates. The
  portable core encodes both base stylus (position, pressure, tilt)
  ...
  installation, hardware decode, HUKS-backed secure pairing, production
