...
final class ProtocolV1SessionCoordinator {
    ...
    private func renegotiateSelectedDisplayLocked(
        ...
        if let failure = routeDisplayFailure(displayID: displayID, streamID: streamID, correlationID: correlationID) {
            return failure
        }
    ...
    private func handleControlLocked(_ bytes: Data) -> [ProtocolV1SessionAction] {
        ...
        case .startDisplayRequest(let request):
            ...
            if actions.contains(where: { if case .close = $0 { true } else { false } }) {
                return actions
            }
            return [.selectDisplay(id: requestedID)] + actions
