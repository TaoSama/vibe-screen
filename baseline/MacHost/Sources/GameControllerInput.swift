...
final class IOKitVirtualGamepadDevice: VirtualGamepadDevice {
    ...
    deinit {
        // Best-effort cleanup when close() was not called. We must NOT use
        // queue.sync here: deinit can run on any thread, and syncing to the
        // device's own queue would deadlock if that queue is the current one.
        // IOHIDUserDeviceCancel (via handle.cancel()) is safe to call without
        // the queue and is idempotent. The neutral report is intentionally
        // skipped on this path; the normal close() path owns it.
        handle.cancel()
    }
...
final class GameControllerStateMachine {
    ...
    static let defaultMaximumControllers = 4
