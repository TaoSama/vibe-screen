...
final class VideoEncoderInFlightAdmission {
    ...
    var snapshot: Snapshot {
        lock.lock()
        defer { lock.unlock() }
        return Snapshot(inFlight: state.activeIdentifiers.count, capacity: capacity)
    }
...
final class VideoEncoderCallbackOwner {
    ...
    func requestKeyframeIfActive() {
        claimEncoder()?.requestKeyframe()
    }
}

final class VideoEncoderKeyframeRequestState {
    private struct State {
    ...
    func requestKeyframe() {
        lock.withLock { $0.pendingForceKeyframe = true }
    }
    ...
    func restoreConsumedRequest(_ consumed: Bool) {
        guard consumed else { return }
        requestKeyframe()
    }

    var hasPendingRequest: Bool {
        lock.withLock { $0.pendingForceKeyframe }
    }
}

enum VideoEncoderCallbackFailureRecovery {
    static func requestKeyframe(owner: VideoEncoderCallbackOwner) {
        owner.requestKeyframeIfActive()
    }
}
...
enum VideoEncoderCallbackLifecycle {
    static func process(
        _ claimedFrame: VideoEncoderFrameRegistry.ClaimedFrame,
        _ body: (VideoEncoderFrameRegistry.ClaimedFrame) -> Void
    ) {
        defer { claimedFrame.context.completeSubmission() }
        body(claimedFrame)
    }
}
...
class VideoEncoder {
    ...
    private let keyframeRequestState = VideoEncoderKeyframeRequestState()
    ...
    var hasPendingKeyframeRequest: Bool { keyframeRequestState.hasPendingRequest }
    ...
    var inFlightCount: Int { inFlightSnapshot.inFlight }
    ...
    func updateSettings(
        ...
        keyframeRequestState.requestKeyframe()
    ...
    func encode(
        ...
        var consumedForceKeyframeRequest = false
        let submissionResult = inFlightAdmission.submit { admissionLease in
            let shouldForceKeyframe = keyframeRequestState.consumePendingRequest()
            consumedForceKeyframeRequest = shouldForceKeyframe
        ...
        if case .submitted(let encodeStatus) = submissionResult, encodeStatus != noErr {
            ...
            keyframeRequestState.restoreConsumedRequest(consumedForceKeyframeRequest)
...
enum VideoEncoderAnnexBConverter {
    static func appendLengthPrefixedNALUnits(
        from dataPointer: UnsafeRawPointer,
        lengthAtOffset: Int,
        totalLength: Int,
        to frameData: inout Data
    ) -> Bool {
        guard totalLength >= 0,
              lengthAtOffset >= totalLength else {
            return false
        }

        var offset = 0
        while offset < totalLength {
            guard totalLength - offset >= MemoryLayout<UInt32>.size else {
                return false
            }

            var nalLength: UInt32 = 0
            memcpy(&nalLength, dataPointer.advanced(by: offset), MemoryLayout<UInt32>.size)
            nalLength = UInt32(bigEndian: nalLength)
            offset += MemoryLayout<UInt32>.size

            let nalByteCount = Int(nalLength)
            guard nalByteCount <= totalLength - offset else {
                return false
            }

            frameData.append(contentsOf: nalStartCode)
            frameData.append(
                dataPointer.advanced(by: offset).assumingMemoryBound(to: UInt8.self),
                count: nalByteCount
            )
            offset += nalByteCount
        }
        ...
        VideoEncoderCallbackLifecycle.process(claimedFrame) { claimedFrame in
            guard status == noErr,
                ...
                if status != noErr {
                    ...
                    VideoEncoderCallbackFailureRecovery.requestKeyframe(owner: claimedFrame.owner)
                }
                return
            }
            ...
            guard VideoEncoderAnnexBConverter.appendLengthPrefixedNALUnits(
                from: UnsafeRawPointer(dataPointer),
                lengthAtOffset: lengthAtOffset,
                totalLength: totalLength,
                to: &frameData
            ) else {
                debugLog("VideoToolbox encode callback produced malformed NAL data")
                return
            }
