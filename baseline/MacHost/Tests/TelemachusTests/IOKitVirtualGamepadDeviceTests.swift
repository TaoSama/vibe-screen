...
final class IOKitVirtualGamepadDeviceTests: XCTestCase {
    ...
    func testDeinitBestEffortSubmitsNeutralBeforeCancelling() throws {
        let provider = FakeVirtualGamepadIOProvider()
        var device: IOKitVirtualGamepadDevice? = try IOKitVirtualGamepadDevice(
            controllerID: "c1",
            controllerEpoch: 1,
            provider: provider
        )
        let handle = try XCTUnwrap(provider.createdHandle)
        let cancelled = expectation(description: "deinit cancelled the virtual device")
        handle.setOnCancel { cancelled.fulfill() }
        device = nil
        wait(for: [cancelled], timeout: 1)

        XCTAssertNil(device)
        XCTAssertEqual(handle.submittedReports.last, try GameControllerHIDReport.encode(.neutral))
        XCTAssertEqual(handle.cancelCount, 1)
    }

    func testPropertiesDescribeTheExpectedVirtualGamepad() throws {
        let provider = FakeVirtualGamepadIOProvider()
        _ = try IOKitVirtualGamepadDevice(
            controllerID: "c1",
            controllerEpoch: 1,
            provider: provider
        )
        let properties = try XCTUnwrap(provider.lastProperties)

        XCTAssertEqual(properties[kIOHIDVendorIDKey] as? Int, 0x5653)
        XCTAssertEqual(properties[kIOHIDProductIDKey] as? Int, 0x0001)
        XCTAssertEqual(
            properties[kIOHIDPrimaryUsagePageKey] as? Int,
            Int(kHIDPage_GenericDesktop)
        )
        XCTAssertEqual(
            properties[kIOHIDPrimaryUsageKey] as? Int,
            Int(kHIDUsage_GD_GamePad)
        )
        XCTAssertEqual(properties[kIOHIDProductKey] as? String, "Vibe Screen Controller")
        XCTAssertEqual(
            properties[kIOHIDReportDescriptorKey] as? Data,
            GameControllerHIDReport.descriptor
        )
    }
    ...
    func testSerialNumberDoesNotLeakOriginalControllerID() throws {
        let provider = FakeVirtualGamepadIOProvider()
        let controllerID = "device-serial-12345"
        _ = try IOKitVirtualGamepadDevice(controllerID: controllerID, controllerEpoch: 1, provider: provider)
        let serial = provider.lastProperties?[kIOHIDSerialNumberKey] as? String

        XCTAssertNotNil(serial)
        XCTAssertFalse(serial?.contains(controllerID) ?? true)
        XCTAssertEqual(serial?.count, 32)
    }
}
