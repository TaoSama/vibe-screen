...
internal class StreamProtocolActionDispatcher(
    ...
    interface Sink {
        ...
        fun onDisplaySelectionConfirmed(selectedId: String)
        ...
        for (action in actions) {
            when (action) {
                ...
                is ProtocolV1Session.Action.DisplaySelectionPending ->
                    sink.onDisplaySelectionPending(action.selectedId, action.pendingId)
                is ProtocolV1Session.Action.DisplaySelectionConfirmed ->
                    sink.onDisplaySelectionConfirmed(action.selectedId)
                ...
                is ProtocolV1Session.Action.ManagedPolicyReceived ->
                    sink.onManagedPolicyReceived(out, session, connectionGeneration, action.status)
