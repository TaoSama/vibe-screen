...
class MainActivity : AppCompatActivity() {
    ...
    private val incomingFileExportCoordinator by lazy { IncomingFileExportCoordinator.processShared(applicationContext) }
    ...
    private var incomingFileExportSubscription: IncomingFileExportSubscription? = null
    ...
    private fun showUsbConnectionGuidance(guidance: ConnectionGuidance) {
        runOnUiThread {
            binding.connectionErrorTitle.text = guidanceStatus(guidance)
    ...
    private fun clearUsbConnectionGuidance() {
        ...
        binding.connectionErrorTitle.setText(R.string.connection_issue)
    ...
    private fun finishIncomingFileTransferState(transferId: ByteString): Boolean {
        if (activeIncomingFileTransfer?.transferId != transferId) return false
    ...
    private fun beginOutgoingFileTransferState(
        ...
        if (activeIncomingFileTransfer != null) {
            cancel(transferId)
            return false
        }
    ...
    private fun activeInternetFileTransferSession(): ActiveFileTransferSession? {
        ...
        val generation = productSessionCoordinator.currentInternetGeneration()
        fun isCurrentAndAllowed(): Boolean =
        ...
        if (generation <= 0L || session.state != InternetProductSessionState.ACTIVE || !isCurrentAndAllowed()) {
            return null
        }
        return ActiveFileTransferSession(
    ...
    private fun handleIncomingFileExportResult(
        ...
        replaceIncomingFileExportSubscription(
            incomingFileExportCoordinator.export(
                IncomingFileExportRequest(pending.source, destination, pending.displayName, pending.mimeType),
                ::handleIncomingFileExportCompletion,
            ),
        )
    }
    ...
    private fun replaceIncomingFileExportSubscription(subscription: IncomingFileExportSubscription) {
        incomingFileExportSubscription?.close()
        incomingFileExportSubscription = subscription
    }

    private fun handleIncomingFileExportCompletion(
        subscription: IncomingFileExportSubscription,
        result: IncomingFileExportResult,
    ) {
        binding.root.post {
            if (isFinishing || isDestroyed || isChangingConfigurations) return@post
            if (incomingFileExportSubscription !== subscription) return@post
            subscription.close()
            ...
            result.export
                .onSuccess {
                    ...
                    showDedupedToast(
                        getString(R.string.file_transfer_copy_saved, result.request.displayName),
                        Toast.LENGTH_LONG,
                    )
                }.onFailure { failure ->
                    subscription.consume()
                    ...
                    showDedupedToast(
                        getString(R.string.file_transfer_copy_failed, result.request.displayName),
                        Toast.LENGTH_LONG,
                    )
                }
        }
    ...
    private fun revokeInternetPairing(reason: String, tombstonePersisted: Boolean = false) {
        val profile = internetProfileStore.loadPublicProfileForRecovery()
