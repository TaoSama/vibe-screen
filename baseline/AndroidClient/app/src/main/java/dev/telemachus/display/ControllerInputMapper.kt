...
internal sealed interface ControllerConnectResult {
    ...
    data class Connected(val dispatch: ControllerDispatch) : ControllerConnectResult

    /** The controller is already active, so no lifecycle event is emitted. */
    data object AlreadyActive : ControllerConnectResult

    /** The active-controller limit rejected this controller. */
    data object LimitReached : ControllerConnectResult
}
...
internal class ControllerSessionState {
    ...
    /**
     * Admits [controllerId] and atomically distinguishes an existing controller from a full session.
     * Callers must handle [ControllerConnectResult.LimitReached] instead of checking [isFull] first.
     */
    fun connect(controllerId: String): ControllerConnectResult = synchronized(lock) {
        if (controllerId in active) return@synchronized ControllerConnectResult.AlreadyActive
        val controller = attach(controllerId) ?: return@synchronized ControllerConnectResult.LimitReached
        ControllerConnectResult.Connected(
            ControllerDispatch(
    ...
    /** Returns an observational UI snapshot; it must not be used as an admission precondition. */
