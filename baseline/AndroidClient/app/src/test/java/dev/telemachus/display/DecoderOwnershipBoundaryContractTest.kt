...
class DecoderOwnershipBoundaryContractTest {
    ...
    fun `main session stream stats consume decoder telemetry through presentation owner`() {
        ...
        assertTrue(decoderTelemetryBlock.contains("currentSessionDecoderTelemetrySnapshot("))
        assertTrue(decoderTelemetryBlock.contains("isCurrentSession(callbackClient, callbackGeneration)"))
        assertTrue(activity.contains("decoderPresentationOwner.currentDecoderSnapshot { decoder -> decoder.consumeTelemetrySnapshot() }"))
