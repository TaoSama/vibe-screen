...
final class ManagedPolicyTests: XCTestCase {
    ...
    func testManagedConfigurationSchemaRejectsMalformedMaximumFileBytes() {
    ...
    func testFileTransferAllowedWithZeroMaximumIsDeniedAndExplained() {
        ...
        XCTAssertTrue(ManagedPolicy.validateRestrictionResults(policy.protocolStatus))
        let fileTransferResult = policy.protocolStatus.restrictionResults.first {
            $0.restriction == ManagedRestrictionResult.fileTransfer
        }
        XCTAssertEqual(fileTransferResult?.allowed, false)
    }

    func testValidateRestrictionResultsTreatsNonEmptyAllowedHostsAsRestricted() {
        var status = managedPolicy(allowedHosts: ["host.local"], deniedHosts: ["host.local"]).protocolStatus
        status.allowedHostsRestricted = false
        let index = status.restrictionResults.firstIndex {
            $0.restriction == ManagedRestrictionResult.allowedHosts
        }!
        status.restrictionResults[index].allowed = true

        XCTAssertFalse(ManagedPolicy.validateRestrictionResults(status))
    }
