...
class Phase3InternetSoakTests(unittest.TestCase):
    ...
    def test_manifest_rejects_malformed_sha256_inputs(self) -> None:
        repository_state_patcher = patch(
            "vibescreen_evidence.phase3_internet_soak.repository_state",
            return_value={"revision": "abc1234", "dirty": False, "status_porcelain": []},
        )
        self.addCleanup(repository_state_patcher.stop)
        repository_state_patcher.start()
        for field, value in (
            ("tls_certificate_sha256", "not-a-sha256-digest"),
            ("android_artifact_sha256", "z" * 64),
        ):
            kwargs = {
                "repo": self.root,
                "command": ["make", "phase3-internet-soak-gate"],
            ...
            kwargs[field] = value
            with self.subTest(field=field), self.assertRaisesRegex(Exception, "SHA-256 digest"):
                build_manifest(**kwargs)

    def test_gate_rejects_malformed_manifest_digests(self) -> None:
        inputs = self.complete_inputs()
        for manifest_field in ("tls_certificate_sha256", "android_artifact_sha256"):
            manifest = self.manifest()
            if manifest_field == "tls_certificate_sha256":
                manifest["deployment"][manifest_field] = "not-a-real-sha256"
            else:
                manifest["build"][manifest_field] = "not-a-real-sha256"
            _write(inputs["manifest_path"], manifest)

            gate = derive_gate(**inputs)

            self.assertEqual(gate["verdict"], "blocked")
            self.assertFalse(gate["criteria"]["manifest"]["passed"])
            self.assertIn(
                "android_artifact_sha256" if manifest_field == "android_artifact_sha256" else "tls_fingerprint",
                gate["criteria"]["manifest"]["checks"],
            )
