...
from vibescreen_evidence.soak_report import EvidenceInputError, derive_report, main
...
class SoakReportTest(unittest.TestCase):
    ...
    def test_derives_exact_window_metrics_and_slopes(self):
        with tempfile.TemporaryDirectory() as raw_directory:
            summary, samples, telemetry = self._write_inputs(Path(raw_directory))
        ...
        self.assertEqual(report["metrics"]["stream"]["reported_dropped_frames"]["sum"], 2.0)
        self.assertEqual(report["metrics"]["telemetry"]["accepted_heartbeat_count"], 2)
        self.assertEqual(report["metrics"]["telemetry"]["stream_stats_gaps"]["maximum_interval_seconds"], 120.0)
        self.assertEqual(report["metrics"]["memory_kib"]["host_rss"]["slope_kib_per_minute"]["full_window"], 10.0)
        self.assertEqual(report["metrics"]["memory_kib"]["host_rss"]["slope_kib_per_minute"]["second_half"], 10.0)
        self.assertEqual(report["metrics"]["memory_kib"]["client_total_pss"]["final"], 280.0)
        self.assertEqual(report["metrics"]["thermal"]["sensors_celsius"]["battery"]["max"], 40.0)
        self.assertEqual(report["metrics"]["battery"]["temperature_celsius"]["min"], 37.0)
    ...
    def test_invalid_lines_and_missing_window_data_are_partial(self):
        with tempfile.TemporaryDirectory() as raw_directory:
            ...
            telemetry.write_text(
                json.dumps({"wall_time": "not-a-time", "event": "stream_stats"}) + "\n",
                encoding="utf-8",
            )
            report = derive_report(summary, samples, telemetry)

        self.assertEqual(report["derivation_status"], "partial")
        self.assertTrue(any("no records in the summary exact window" in error for error in report["errors"]))
