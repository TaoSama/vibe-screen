...
class AV1CurrentBaseGateTests(unittest.TestCase):
    ...
    def test_gate_document_names_current_base_owner_and_blocked_evidence(self) -> None:
        ...
        self.assertIn("2026-08-29", gate)
        self.assertIn("b54ee0e929c53459e6ba7e060f2c9de0c846f408", gate)
        self.assertIn("real AV1 stream still blocked", p0110_current_base_refresh)
        self.assertIn("nubia P0110", p0110_current_base_refresh)
        self.assertIn("pacific", p0110_current_base_refresh)
        self.assertIn("Android 16", p0110_current_base_refresh)
        self.assertIn("SDK 36", p0110_current_base_refresh)
        self.assertIn("<redacted-device-serial>", p0110_current_base_refresh)
        self.assertIn("/tmp/vibe-screen-android-<redacted-device-serial>.lock", p0110_current_base_refresh)
        self.assertIn("No `/usr/bin/sfltool dumpbtm` command was executed", normalized_p0110_current_base_refresh)
        self.assertIn("no login-item diagnostic opt-in argument was used", normalized_p0110_current_base_refresh)
        self.assertIn("c2.qti.av1.decoder", p0110_current_base_refresh)
        self.assertIn("c2.android.av1-dav1d.decoder", p0110_current_base_refresh)
        self.assertIn("status=-12908 session=false", p0110_current_base_refresh)
