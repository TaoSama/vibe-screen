...
from .ios_native_input import (
    ...
    GATE_PROFILE as NATIVE_INPUT_GATE_PROFILE,
...
def _native_input_blocked_gate(
    *,
    provided: bool,
    path: Path | None,
...
def _schema_safe_native_input_owner(owner: Any) -> dict[str, Any] | None:
    ...
    if (
        ...
        and isinstance(owner.get("scope"), str)
        and bool(owner["scope"].strip())
    ):
        return {
...
def _schema_safe_native_input_current_base(current_base: Any) -> dict[str, Any] | None:
    if not isinstance(current_base, dict):
        return None
    commit = current_base.get("commit")
    return {
        "commit": commit.lower() if isinstance(commit, str) and COMMIT_RE.fullmatch(commit) else None,
        "dirty": current_base.get("dirty") if isinstance(current_base.get("dirty"), bool) else None,
    }


def _string_or_object_list(value: Any) -> list[Any]:
...
def _load_native_input_gate(path: Path | None, repository: dict[str, Any]) -> dict[str, Any]:
    if path is None:
        return _native_input_blocked_gate(
            provided=False,
            path=None,
    ...
    missing = _string_or_object_list(document.get("missing_requirements"))
    blocking_reasons = _string_or_object_list(document.get("blocking_reasons"))
    disallowed_evidence = _string_or_object_list(document.get("disallowed_evidence"))
    artifact_paths = _string_list(document.get("artifact_paths"))

    owner_valid = _schema_safe_native_input_owner(owner) is not None
    ...
    requirements_present = (
        document.get("requires_real_ios_device") is True
    ...
    can_close = (
        ...
        and document.get("kind") == NATIVE_INPUT_GATE_KIND
        ...
        and isinstance(document.get("observations"), dict)
        and not missing
    ...
    if not owner_valid:
        missing = [*missing, {"field": "owner", "requirement": "ios native-input gate owner must be the dedicated current-base native-input owner"}]
    if not current_base_matches:
        missing = [*missing, {"field": "current_base", "requirement": "ios native-input gate current_base must match clean repository HEAD"}]
    if not requirements_present:
        missing = [*missing, {"field": "requirements", "requirement": "ios native-input gate must preserve real-device, signed-app, accessory, and substitution constraints"}]
    if not artifact_paths:
        missing = [*missing, {"field": "artifact_paths", "requirement": "retain sanitized iOS native-input artifacts"}]

    if not can_close and not blocking_reasons:
        blocking_reasons = missing

    return {
        "provided": True,
        "path": str(path),
        "owner": _schema_safe_native_input_owner(owner),
        "current_base": _schema_safe_native_input_current_base(current_base),
        ...
        "missing_requirements": [] if can_close else missing,
        "blocking_reasons": [] if can_close else blocking_reasons,
