...
ANDROID_SERIAL_FIELDS = {"adb_serial", "device_serial"}
...
def _public_android_device(android: Any) -> dict[str, Any] | None:
    if not isinstance(android, dict):
        return None
    return {key: value for key, value in android.items() if key not in ANDROID_SERIAL_FIELDS}
...
def build_summary(
    *,
    ...
    android = _public_android_device(preflight.get("android_device"))
    return {
