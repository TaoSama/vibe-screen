"""Summarize Phase 2 stand, charging, thermal-load, and power evidence.

This gate consumes explicit observations plus derived measurements from a Phase
2 tablet run. It does not collect device data and it does not close the separate
eight-hour stream or memory gates by itself. Missing observations default to
false so phone substitutes, short readiness checks, and incomplete artifact
...
GATE_KIND = "phase2_device_environment_acceptance"
...
REQUIRED_FIELDS: tuple[tuple[str, str], ...] = (
    ...
    (
        ...
        "label evidence with the observed device identity without relabeling substitutes",
    ),
    (
...
def _maximum_criterion(measurements: dict[str, Any], field: str, maximum: float) -> dict[str, Any]:
...
def _minimum_criterion(measurements: dict[str, Any], field: str, minimum: float) -> dict[str, Any]:
    measured = _finite_number(measurements.get(field), f"measurements.{field}")
...
def summarize(record: dict[str, Any], *, run_id: str | None = None) -> dict[str, Any]:
    ...
    blocking_reasons = [
        item for item in missing_requirements if item["field"] in BLOCKING_FIELDS
    ]
    ...
    criteria = {
        "environment_duration_seconds": _minimum_criterion(measurements, "environment_duration_seconds", resolved_thresholds["minimum_environment_duration_seconds"]),
        "maximum_sample_gap_seconds": _maximum_criterion(measurements, "maximum_sample_gap_seconds", resolved_thresholds["maximum_sample_gap_seconds"]),
        "unplugged_sample_count": _maximum_criterion(measurements, "unplugged_sample_count", resolved_thresholds["maximum_unplugged_sample_count"]),
        "non_charging_sample_count": _maximum_criterion(measurements, "non_charging_sample_count", resolved_thresholds["maximum_non_charging_sample_count"]),
        "power_source_change_count": _maximum_criterion(measurements, "power_source_change_count", resolved_thresholds["maximum_power_source_change_count"]),
        "maximum_thermal_status": _maximum_criterion(measurements, "maximum_thermal_status", resolved_thresholds["maximum_thermal_status"]),
        "maximum_battery_temperature_celsius": _maximum_criterion(measurements, "maximum_battery_temperature_celsius", resolved_thresholds["maximum_battery_temperature_celsius"]),
        "net_battery_drain_percent": _maximum_criterion(measurements, "net_battery_drain_percent", resolved_thresholds["maximum_net_battery_drain_percent"]),
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("input", help="phase2 device-environment observations .json file, or - for stdin")
