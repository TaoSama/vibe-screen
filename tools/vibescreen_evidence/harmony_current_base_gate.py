...
MatePad Mini, hardware decode, and Host resume interoperability evidence exists.
"""
...
OWNER_GATES = {
    "hardware_decode_capability": {
        ...
        "readiness_prerequisites": ("deveco", "hap", "matepad", "host"),
        "requirement": "H.264 and HEVC hardware decoder identity plus rendered-frame evidence on a HarmonyOS MatePad Mini",
    },
    "hap_install": {
        "owner": "HarmonyOS HAP lifecycle evidence",
        "required_device_gates": ("signed_release_hap", "hap_install_launch"),
        "required_evidence_markers": ("harmony-hap-readiness.json",),
        "readiness_prerequisites": ("deveco", "hap", "matepad"),
...
INTERPRETATION = (
    "A pass means the README Phase 4 owner gates for HarmonyOS hardware "
    "decode capability, HAP install, and resume-capable Host interoperability "
    "are backed by a passing readiness preflight and passing MatePad Mini "
    "device-gate manifest. Missing DevEco, signed HAP, MatePad Mini, Host "
    "identity, or gate evidence remains blocked/open and is not acceptance "
    "evidence."
)
...
def _readiness_checks(readiness: dict[str, Any] | None) -> dict[str, dict[str, Any]]:
    ...
    hap_ready = (
        ...
        and all(
            _non_empty_string(artifact.get(key))
    ...
    host_ready = _non_empty_string(host_commit) and _non_empty_string(host_build) and host.get("protocol") == "Protocol v1"
...
def _device_gate_checks(device_gates: dict[str, Any] | None) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    metadata["protocol_v1_host"] = _check(
        ...
        and _non_empty_string(host.get("commit"))
        and _non_empty_string(host.get("build_sha256")),
...
def derive_gate(readiness_path: Path, device_gates_path: Path) -> dict[str, Any]:
    ...
    device_metadata, owner_gate_checks = _device_gate_checks(device_gates)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = derive_gate(args.readiness, args.device_gates)
    try:
