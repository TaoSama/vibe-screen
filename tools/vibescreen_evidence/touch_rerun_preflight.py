...
def collect_current_source(source_root: Path) -> dict[str, Any]:
    ...
    commit = _run(["git", "rev-parse", "HEAD"], cwd=root).stdout
    tree = _run(["git", "rev-parse", "HEAD^{tree}"], cwd=root).stdout
    status = _run(["git", "status", "--porcelain"], cwd=root).stdout
...
def _query_tcc_db_direct(db_path: Path, bundle_identifier: str) -> dict[str, dict[str, Any]]:
    ...
    for service, client, client_type, auth_value, auth_reason, last_modified, csreq in rows:
        ...
        current = result.get(service)
        row_modified = last_modified if last_modified is not None else -1
        current_modified = (current or {}).get("last_modified", -1)
        if current is not None and row_modified < current_modified:
            continue
        result[service] = {
            "service": service,
            "client": client,
...
def build_blocked_error_document(
    ...
    *,
    ...
    expected_android_release: str | None = None,
    expected_android_sdk: int | None = None,
    current_source_required: bool = False,
) -> dict[str, Any]:
    return {
        "schema_version": SCHEMA_VERSION,
        "kind": "touch_fixed_binary_rerun_preflight",
        ...
        if arguments.output is not None:
            write_json(
                ...
                build_blocked_error_document(
                    ...
                    expected_android_release=arguments.expected_android_release,
                    expected_android_sdk=arguments.expected_android_sdk,
                    current_source_required=arguments.require_current_source,
                ),
            )
