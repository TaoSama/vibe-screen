...
ios-videotoolbox-readiness:
	@test -f "$(EVIDENCE_DIR)/ios-videotoolbox-observations.json" || (echo "error: collect $(EVIDENCE_DIR)/ios-videotoolbox-observations.json before ios-videotoolbox-readiness" >&2; exit 2)
	PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=tools python3 -m vibescreen_evidence.ios_videotoolbox_readiness $(EVIDENCE_DIR)/ios-videotoolbox-observations.json --output $(EVIDENCE_DIR)/ios-videotoolbox-readiness.json
