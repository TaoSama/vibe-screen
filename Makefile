...
ios-native-input-gate:
	@test -f "$(EVIDENCE_DIR)/ios-native-input-observations.json" || (echo "error: collect $(EVIDENCE_DIR)/ios-native-input-observations.json before ios-native-input-gate" >&2; exit 2)
	PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=tools python3 -m vibescreen_evidence.ios_native_input $(EVIDENCE_DIR)/ios-native-input-observations.json --output $(EVIDENCE_DIR)/ios-native-input-gate.json
