...
macos-startup-recovery-gate:
	@test -f "$(EVIDENCE_DIR)/macos-startup-recovery-observations.json" || (echo "error: collect $(EVIDENCE_DIR)/macos-startup-recovery-observations.json before macos-startup-recovery-gate" >&2; exit 2)
	PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=tools python3 -m vibescreen_evidence.macos_startup_recovery_gate "$(EVIDENCE_DIR)/macos-startup-recovery-observations.json" --output "$(EVIDENCE_DIR)/macos-startup-recovery-summary.json"
