...
HARMONY_HOST_INTEROP_PREFLIGHT_JSON ?= $(EVIDENCE_DIR)/harmony-host-interop-preflight.json
PHASE3_HOST_LOG ?=
PHASE3_ANDROID_LOG ?=
PHASE3_DEVICE_INFO ?=
...
phase3-internet-soak-manifest:
	...
	@test -n "$(strip $(PHASE3_INTERNET_TURN_URI))" || (echo "error: set PHASE3_INTERNET_TURN_URI" >&2; exit 2)
...
phase3-internet-soak-gate:
	...
	PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=tools \
		...
		--manifest "$(PHASE3_INTERNET_SOAK_MANIFEST_JSON)" \
...
harmony-hap-readiness:
	@test -n "$(strip $(EVIDENCE_DIR))" || (echo "error: set EVIDENCE_DIR to a HarmonyOS HAP lifecycle readiness evidence directory" >&2; exit 2)
