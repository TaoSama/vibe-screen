...
class SharedProtocolModelVerifierTest(unittest.TestCase):
    ...
    def test_manifest_argument_accepts_external_absolute_path(self) -> None:
    ...
    def test_manifest_fails_closed_when_envelope_payload_is_missing(self) -> None:
        document = json.loads(MANIFEST.read_text(encoding="utf-8"))
        document["envelopePayloads"].pop("peripheral_event")
        result = self.run_with_manifest(document)
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("Envelope payload field numbers", result.stderr)

    def test_manifest_fails_closed_when_capability_is_missing(self) -> None:
        document = json.loads(MANIFEST.read_text(encoding="utf-8"))
        document["capabilities"].pop("CAPABILITY_PERIPHERAL_INPUT_FRAMEWORK")
        result = self.run_with_manifest(document)
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("Capability enum values", result.stderr)
